...
public class VideoFileManager {
	...
	private Path getVideoPath(Long id){
		assert(id != null);
		
		return targetDir_.resolve("video"+id+".mpg");
	}
	 ...
	 * @return
	 */
	public boolean hasVideoData(Long id){
		Path source = getVideoPath(id);
	 ...
	 * @throws IOException 
	 */
	public void copyVideoData(Long id, OutputStream out) throws IOException {
		Path source = getVideoPath(id);
		if(!Files.exists(source)){
			throw new FileNotFoundException("Unable to find the referenced video file for videoId:"+id);
		}
	 ...
	 * @throws IOException
	 */
	public void saveVideoData(Long id, InputStream videoData) throws IOException{
		assert(videoData != null);
		
		Path target = getVideoPath(id);
